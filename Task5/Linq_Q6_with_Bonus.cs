namespace LINQ_Q6_with_Bonus
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            var BonusQ = Employees.Select(e => new
            {
                Name = e.Name,
                Dept = e.Department,
                SeniortyLevel = (e.Salary) switch
                {
                    >= 9000 => "Senior",
                    >= 7000 => "Mid",
                    _ => "Junior"
            ...
            foreach (var item in BonusQ)
            {
                Console.WriteLine($"Name: {item.Name} - Dept: {item.Dept} - Seniority: {item.SeniortyLevel} ,");
            }
        }
